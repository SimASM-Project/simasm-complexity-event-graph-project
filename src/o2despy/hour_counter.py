...
class HourCounter(IHourCounter):
    ...
    def working_time_ratio(self):
        ...
        if self._last_time == self._initial_time:
            return 0
        hours = _timedelta2hours(self._last_time - self._initial_time)
        try:
    ...
    def history(self):
        ...
        if self._keep_history:
            records = [
                (_timedelta2hours(key - self._initial_time), self._history[key])
    ...
    def observe_count(self, count, clock_time=None):
        ...
        if not self._paused:
            hours = _timedelta2hours(clock_time - self._last_time)
