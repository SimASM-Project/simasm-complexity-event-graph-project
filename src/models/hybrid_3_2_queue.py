...
class Hybrid32Queue(Sandbox):
    def __init__(self, seed=0, code="Hybrid32Queue", capacity=5, iat_mean=1.25, ist_mean=1.0):
    ...
    def _update_areas(self):
        ...
        if self._last_event_time is not None:
            ...
            if dt > 0:
                self._area_queue_total += (self.queue_count_1 + self.queue_count_2 + self.queue_count_3) * dt
    ...
    def join(self):
        self._update_areas()
        if self.part_1_num > 0 and self.part_2_num > 0:
            self.part_1_num -= 1
            self.part_2_num -= 1
